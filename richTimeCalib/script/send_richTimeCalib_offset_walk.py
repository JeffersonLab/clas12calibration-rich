...
def add_hist_job(wf,fnl,phase=0,c=0):
    global outdir_pref, appHist, scriptHist, RN, envscript, TOFILE, TWFILE
    script = scriptHist
    jname = wf + "_" + "ph" + str(phase) + "_" + str(c)
    size = 0
    for fname in fnl:
        size += os.path.getsize(fname)/1024./1024.*1.15
    cmd  = "swif add-job -workflow " + wf + " -ram 9000mb -project clas12 -time 6h -disk " + "{0:.0f}".format(size) + "mb "
    cmd += " -track "+ TRACK
    cmd += " -phase " + str(phase)
    cmd += " -name " + jname
    cmd += " -shell /bin/bash" 
    ...
    cmd += " ./" + script.split("/")[-1] + " " + str(T) +  " " + outdir + " " + outpref + " " + RN + " -tow " + TOFILE.split('/')[-1] + " " + TWFILE.split('/')[-1]
    if DEBUG : print (cmd)
    subprocess.call(cmd,shell=True)
...
def add_ana_job(wf,flist,phase=0):
    ...
    corr_type = ["TCheck"]
    outdir = outdir_pref + "/" + corr_type[ind] + "OW"
    ...
    for fnl in flist:
        ...
        fname = outdir_pref + "/T"+ str(T) +"OW/" + fname + "__RichTimeCalibE_" + RN + "_" + str(T) +".root"
        cmd += " -input " + fname.split("/")[-1] + " file:" + fname
        c +=1
...
def main():
    global WF, RN, outdir_pref, MAXJOBS, BS, TOFILE, TWFILE, RICHTIMECALIB_OUTPATH
    if len(argv)!=5: 
        print ("A directory with calibration files or filelist must be supplied together with a run number a timeoffset parameters file and a timewalk parameters file")
        print (argv[0] + " filelist.txt <RN> <TOFILE> <TWFILE>")
        exit (1)
    ...
    ##### Setting jobs phase ###########
    phase = 0
    ##### Putting jobs to make histograms using timeoffset and timewalk parameters ####
    add_hist_job_bunch(flist,phase)

    ##### Setting jobs phase ###########
    phase +=1
    #### Putting jobs to check the corrections #########
